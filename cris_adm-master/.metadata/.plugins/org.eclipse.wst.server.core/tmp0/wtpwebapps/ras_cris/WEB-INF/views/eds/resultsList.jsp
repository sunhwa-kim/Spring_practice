...
<head>
<link rel="stylesheet"
	href="${ctxt}/resources/css/ncmik/edsStyle/style.css" type="text/css"
	media="screen" />
<link rel="stylesheet"
	href="${ctxt}/resources/css/ncmik/edsStyle/pubtype-icons.css"
...
<script>
	...
	function fn_edsLink(edsLink, text) {
		...
		if (text.indexOf("Full Text") != -1
				|| text.indexOf("proxy_check") != -1) {

			if (innerIp == 'Y') {
				window.open(replaceAll(edsLink, '+', '%2B'));
			} else {
				if (userId != '') {
					window.open("http://152.99.73.136/_Lib_Proxy_Url/"
							+ replaceAll(edsLink, '+', '%2B'));
				} else {
	...
	$(function() {
		...
		if ('${resultsList.apierrormessage.errorNumber}' == '111') {
			setTimeout(fn_tab(1), 1000);
		}
		...
		$("#applyLimiters")
				.click(
						function() {
							var actions = "";
							$(
									"#limiterOptions input.selected-limiter-option:not(:checked)")
									.each(function() {
										actions += "&" + $(this).val();
									});
							$(
									"#limiterOptions input.unselected-limiter-option:checked")
									.each(function() {
										actions += "&" + $(this).val();
									});
							...
							window.location = [ location.protocol, "//",
									...
									currentPageState, actions ].join('')
									+ "&detailYn=${vo.detailYn}";
							return false;
						});

		$("#applyExpanders").click(
						function() {
							var actions = "";
							$(
		...
		var eng = "";
		var lang_cnt = 0;
		$("#5").siblings().children('li').each(function(cnt) {
			if (cnt == 0) {
		...
		$("#1").siblings().children('li').slice(5).hide();

		$("#2").siblings().children('li').slice(5).hide();

		$("#3").siblings().children('li').slice(5).hide();

		$("#4").siblings().children('li').slice(5).hide();

		$("#5").siblings().children('li').slice(lang_cnt).hide();

		$("#6").siblings().children('li').slice(5).hide();

		$("#7").siblings().children('li').slice(5).hide();

		$("#8").siblings().children('li').slice(5).hide();

		$("#9").siblings().children('li').slice(5).hide();
	...
	function moveYear() {
		...
		var actions = "&action=AddLimiter(DT1:" + $('#f_date').val() + "-01 / "
				+ $('#t_date').val() + "-12)"
		...
		window.location = [ location.protocol, "//", location.host,
				location.pathname, "?", currentPageState, actions ].join('')
				+ "&detailYn=${vo.detailYn}";
	}
	...
	function applyLimiters() {
		...
		$("#limiterOptions input.selected-limiter-option:not(:checked)").each(
			function() {
				actions += "&" + $(this).val();
			});
		$("#limiterOptions input.unselected-limiter-option:checked").each(
			function() {
				actions += "&" + $(this).val();
			});
		var currentPageState = $("#currentState").val();
	...
	function applyFilter(applyFacetURL, checkId) {
		if ($("input[value='" + checkId + "']").is(":checked")) {
			...
			$('.search_param').each(
				function() {
					location.href = $("input[name='" + checkId + "']")
							.val()
							+ "&kwd=" + '${kwd}';
				});
			$('#mloader').show();
	...
	function fn_search() {
		...
		$('#searchKeyword').val($('#keyword').val());
		  ...
		  if($('#searchKeyword').val() != ''){
			 if($('select[name=searchKey] :selected').val() == ''){
				 if(strQuery != ''){strQuery +=' ';}
				 strQuery += $('#searchKeyword').val();
			 }else{
		...
		$('#kwd').val(strQuery); 
		$('#fieldcode').val( $('select[name=searchKey] :selected').val() );
		
		if ($('#resultAdd').is(':checked')) {
			location.href = "AdvancedSearch.do?" + $('#currentState').val() + "&action=AddQuery(AND," + $('#keyword').val() + ")";
		} else {
	...
	function fn_multiSave() {
		...
		$("input[name=chk_mylib]:checked").each(
			function() {
	...
	//전체선택 / 취소
	function fn_all() {
		$("input[name=chk_mylib]").each(
						function() {
							if (isChk) {
								$('#all_chk').html("<i class=\"far fa-check-square\" style=\"vertical-align:bottom;\"></i>전체선택");
								$(this).prop("checked", false);
							} else {
								$('#all_chk').html("<i class=\"far fa-check-square\" style=\"vertical-align:bottom;\"></i>전체취소");
	...
	/* 	var mGbn = "on";
	 ...
	 function fn_changeMenu(){

	 if(mGbn == "on"){
	 $("#accordionMenu1  > li").eq(6).show();
	 $("#accordionMenu1  > li").eq(7).show();
	 $("#accordionMenu1  > li").eq(8).show();
	 $("#accordionMenu1  > li").eq(9).show();
	 mGbn = "off";
	 }else{
	 $("#accordionMenu1  > li").eq(6).hide();
	 $("#accordionMenu1  > li").eq(7).hide();
	 $("#accordionMenu1  > li").eq(8).hide();
	 $("#accordionMenu1  > li").eq(9).hide();	
	 mGbn = "on";
	 }
...
/* #sortoption, #perpageoption, #pageoption{
	margin-top:0;
	height:38px;
	line-height:38px;
	float:none;
...
.info .pubtype .links a, .info .links a img {
	line-height: 28px;
}
...
@
-moz-keyframes masterkey_blink { 15% {
	color: white;
}
...
@
-ms-keyframes masterkey_blink { 15% {
	color: white;
}
...
@
-o-keyframes masterkey_blink { 15% {
	color: white;
}
...
<div class="sub_center_text">
	<div class="sub_center_nav">
		...
		<span> > </span> <span>통합검색</span> <span> > </span> <span
			class="active">전자자원</span>
	</div>
	<span class="center_text_01">전자자원 <span class="category_select">▼</span></span>
...
<div class="content_wrap" style="min-height: 700px;">
	...
	<div class="content_box">
		<div class="content_list">
			...
			<form id="edsForm" name="edsForm" action="${ctxt}/search/AdvancedSearch">
				...
				<div id="search-group" style="margin-top: 0; display: none;">
					<div class="search-input" style="padding-bottom: 3px;">
						...
						<input type="text" name="query" id="searchKeyword" value="" /> 						
			...
			<div class="sub_book_wrap">
				<input type="hidden" id="detailYn" name="detailYn" value="${vo.detailYn}" />
				<div id="eds-group">
					<div class="search-input">
						...
						<input type="text" id="keyword" name="keyword" value="" onkeypress="javascript:if(event.keyCode == 13) { fn_search(); return false;}" />
						<button type='button' id="btnSearch" onclick="fn_search();">
							<i class="fas fa-search"></i>
						</button>
						<span class="media_span"><input type="checkbox" value="action" id="resultAdd" style="vertical-align: bottom; margin-right: 0;">
						<font style="font-size: 11px; width: 62px; vertical-align: bottom;"><label>결과내검색</label></font>
						</span>
					</div>
				</div>
				<%
				...
				<div class="under_bar media_bar relative">
					<div class="page">
					<!-- 	<span class="first">1</span> - <span class="last">10</span> -->
				...
				<div class="clear">
					<div class="button_box">
						...
						<a href="#" class="" onclick="javasctipt:fn_multiSave();"><i class="fas fa-book"></i>내서재담기 </a> 
						<a href="#" class="" onclick="javasctipt:fn_outData();"><i class="far fa-share-square"></i>내보내기</a>
					</div>
							...
							<%
								if (null != info && null != info.getAvailableSortsList()) {
									...
									for (AvailableSort availableSort : availableSortList) {
										...
										if (resultList.getSelectedSort().compareToIgnoreCase(
												availableSort.getId()) == 0) {
											selected = "selected=selected";
										}
							%>
							<option <%=selected%> value="<%=availableSort.getAddAction()%>">
								<%
							...
							<%
								String pageOptionselect1 = "";
									...
									if (null == view || view.isEmpty()) {
										view = (null != info.getViewResultSettings()) ? info
												.getViewResultSettings().getResultListView()
												: "detailed";
									}
                        ...
                        if(!author.equals("")) {	
							...
							<%
								...
								<div class="chkList<%=result.getResultId()%>">
									...
									<input type="hidden" name="edsUrl" value="${ctxt}/search/Retrieve">
									<!-- 내서재 -->
										...
										if (result.getRecordInfo() != null) {
											if (result.getRecordInfo().size() > 0) {
												for (RecordInfo recordInfo : result.getRecordInfo()) {
													...
													//DOI 
													if (recordInfo.getIdentifier_type() != "null"
															&& recordInfo.getIdentifier_type() != null
															&& recordInfo.getIdentifier_type() != "") {
							...
							%>
								...
								<a class="fulltext"
									href="${ctxt}/search/Retrieve?an= <%=result.getAn()%>&dbid=<%=result.getDbId()%>&highlight=<%=URLEncoder.encode(query.getTerm(), "UTF-8")%>&edsCnt=${edsCnt}&kwd=${kwd}#html"><span
									class="btn_name">HTML</span>
								<!--  Full Text --></a>
								<%
									}
								%>
								...
								<%-- <a class="pdf fulltext" style="height: 20px;" href="${ctxt}/search/Retrieve?an= <%=result.getAn()%>&dbid=<%=result.getDbId()%>&highlight=<%=URLEncoder.encode(query.getTerm(), "UTF-8")%>&type=pdf"><img width="70%" src="${ctxt}/resources/images/_cmmn/ico_pdf.png"><!--  Full Text --></a> --%>
									<a class="fulltext" href="javascript:fn_getPdf('${ctxt}/search/getPdf?an= <%=result.getAn()%>&dbid=<%=result.getDbId()%>&highlight=<%=URLEncoder.encode(query.getTerm(), "UTF-8")%>&type=pdf');">
								...
								if (result.getFullTextList() != null) {
									...
									<a class="fulltext" href="javascript:fn_edsLink('<%=reUrl%> ','proxy_check')"><img src="<%=fullTextCustomLink.getIcon()%>"><span class="btn_name"><%=fullTextCustomLink.getText()%></span>
									<!--  Full Text --></a>
									...
									%>
									<a class="fulltext"	href="${ctxt}/search/Retrieve?an=<%=result.getAn()%>&dbid=<%=result.getDbId()%>&highlight=<%=URLEncoder.encode(query.getTerm() , "UTF-8")%>&type=<%=link.getType() %>">
									<span class="btn_name"><%=link.getType()%></span> <!--  Full Text --></a>
									<%
										}
									}
									%>
...
>>>>>>> .r207
				...
				<div id="paging" class="paginate" style="text-align: center;">
					<%
						String kwd = (String) request.getAttribute("kwd").toString()
								.replace("<", "");
						String output = PageNumberHelper.buildPageNumber(
								resultList.getHits(),
								resultList.getSelectedResultsPerPage(),
								resultList.getSelectedPageNumber(), "AdvancedSearch?"
										+ resultList.getQueryString() + "&kwd=" + kwd
										+ "&detailYn=" + resultList.getDetailYn());
					%>
					<%=output%>
				</div>
						...
						<ul style="padding-left: 0; list-style: none; font-size: 12px;">
							<%
								//Create a map of available limiters
									...
									if (null != info && null != info.getAvailableLimitersList()) {
										...
										for (AvailableLimiter al : availableLimiterList) {
											if (al.getType().equals("select")) {
												...
												availableLimiterMap.put(al.getId(),
														al.getLimiterValues());
									...
									ArrayList<LimiterWithAction> selectedLimiterList = resultList
											.getSelectedLimiterList();
									...
									for (int i = 0; i < selectedLimiterList.size(); i++) {
										...
										if (obj instanceof String) {
											...
											ArrayList<LimiterValueWithAction> lvwaList = lwa
													.getLvalist();
							...
							<li><p>
									<a
										href="${ctxt}/search/AdvancedSearch?<%=resultList.getQueryString() %>&action=<%=lwa.getRemoveAction()%>&detailYn=${vo.detailYn}"><img
										src="${ctxt}/resources/css/ncmik/edsStyle/delete.png" /> <%
 	out.println("Limiter: "
 					+ (limiterName == "" ? "pubDate" : limiterName));
 %></a>
								</p></li>
							<%
								}
									...
									ArrayList<ExpandersWithAction> expanderRemoveActionList = resultList
											.getExpanderwithActionList();
							...
							<li><p>
									<a
										href="${ctxt}/search/AdvancedSearch?<%=resultList.getQueryString() + "&action=" + ewa.getRemoveAction()%>&detailYn=${vo.detailYn}"><img
										src="${ctxt}/resources/css/ncmik/edsStyle/style/delete.png" />
										<%
											out.println("Expander: " + label);
										%></a>
								</p></li>
							<%
								}
									ArrayList<FacetFilterWithAction> facetfiltersList = resultList
											.getFacetfiltersList();
									...
									for (int i = 0; i < facetfiltersList.size(); i++) {
										...
										ArrayList<FacetValueWithAction> fvwalist = ffwa
												.getFacetValueWithAction();
							...
							%>
							<li class="search_param"><a
								href="${ctxt}/search/AdvancedSearch?<%=resultList.getQueryString()+"&action=" + removeAction%>+&kwd=${kwd}"
								id="<%=efv.getValue()%>" onclick="chkDel();"> <img
									src="${ctxt}/resources/css/ncmik/edsStyle/delete.png" /> <%
 	out.println(efv.getId() + ": " + efv.getValue());
 %>
							</a> <input type="hidden" id="<%=efv.getValue()%>"
								name="<%=efv.getValue()%>"
								value="${ctxt}/search/AdvancedSearch?<%=resultList.getQueryString()+"&action=" + removeAction%>&detailYn=${vo.detailYn}" />
					...
					<ul class="acc-menu" id="accordionMenu1">
						...
						<li data-extension="close">
							...
							<ul class="sub" id="limiterOptions">
								<%
									if (limiterRemovemap.containsKey(availableLimiter
															.getId())) {
														String removeAction = limiterRemovemap
																.get(availableLimiter.getId());
								%>
								<li name="basic_li"><input class="selected-limiter-option"
									type="checkbox" name="<%=availableLimiter.getId()%>"
									checked="checked" value="<%="action=" + removeAction%>"
									onclick="applyLimiters()" /> <%
 ...
 %>
								...
								%>
								<li name="basic_li"><input
									class="unselected-limiter-option" type="checkbox"
									name="<%=availableLimiter.getId()%>"
									value="<%="action="
										+ availableLimiter.getAddAction()%>"
									onclick="applyLimiters()" /> <%
 	out.println(availableLimiter.getLabel());
 %>
								</li>
						...
						<li data-extension="close">
							<div class="main-title">
								<span class="folder"> </span><a>PubDate</a>
							</div>
							<ul class="sub">
								<li style="text-align: center" class="date">
									...
									name="<%=availableLimiter.getId()%>" value=""></li>
								<li id="moreYear"><a class="more_btn2"
									href="javascript:moveYear();">적용</a></li>
							</ul>
						</li>
						<%
							}
						...
						<%
							ArrayList<Facet> facetsList = resultList.getFacetsList();
								...
								ArrayList<String> arrayList = new ArrayList<String>(
										Arrays.asList("Source Type", "Language"));
						...
						%>
						<li data-extension="close">
							<div class="main-title" id="<%=facetId%>">
							...
							<ul class="sub">
								<%
									ArrayList<FacetValue> facetsValueList = facet
													.getFacetsValueList();
											...
											for (FacetValue facetValue : facetsValueList) {
												...
												String applyFacetURL = "search/AdvancedSearch?"
														+ resultList.getQueryString() + "&action="
														+ facetValue.getAddAction();
								...
								<li><c:set var="facetValue"
										value="<%=facetValue.getValue()%>" scope="session" /> <input
									type="checkbox" id="<%=facetValue.getValue()%>"
									onclick="javascript:applyFilter('<%=applyFacetURL%>','<%=facetValue.getValue()%>');"
									value="<%=facetValue.getValue()%>" /> <%-- <c:if test="${facetValue eq efvValue}"> checked="checked" </c:if> --%>
								...
								%>
								<span><li><a class="more_btn2"
										href="javascript:fn_rightMenu('<%=facetId%>');">더보기</a></li></span>
								<%
									}
								%>
								<%
					...
					<!---------------------------------------- END FACETS ---------------------------------------->
					<!-- <input type="button" value = "Open"  onclick="javascript:fn_changeMenu();" style="border: 1px solid #e6e6e6;width:100%; height: 30px;"  /> -->
...
<form:form name="chkUploadForm" id="chkUploadForm"
