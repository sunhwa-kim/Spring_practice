		...
		"setPubmed": function(pubmedList) {
			...
			if (pubmedList) {
				...
				var midElement = {
					...
					"div": $("<div>", {
						"style": "min-height: 70px; border-bottom:1px solid #CCDDED; margin-bottom:7px;padding-bottom:7px;",
					}),
					"a": $("<a>", {
					...
					"make": function(count, pubmedObj) {
						...
						a.text(title).bind("click", function() {
							// 상세 내용 클릭시 이동 이벤트
							var url =  '${ctxt}/search/searchPubmedDtl.do?uId='+uId;
							location.href=url;
				...
				if (pubmedIds.length) {
					...
					$.each(pubmedIds, function(i, pubmedId) {
						...
						$p.getPubmedInfo(pubmedId, function(pubmedInfo,cnt) {
								...
								var obj = midElement.make(cnt + 1 + retStart, pubmedInfo);
								box1.append(obj);
								var boxHtml = box1.html(); 
								html_array[cnt] = boxHtml;
								total_cnt ++;
								
								if(html_array.length == all_total_cnt){
 									if(isProc){   
										return false;    
									}         
									
									for (var html_cnt = 0; html_cnt < html_array.length; html_cnt ++) {
										if(html_array[html_cnt] == null || html_array[html_cnt] == undefined || html_array[html_cnt] == 'undefined' || html_array[html_cnt] == '' ){
											return true;
										}
									}

									for (var cnt_tot = 0; cnt_tot < all_total_cnt; cnt_tot++) {
										str_html += html_array[cnt_tot];
									}
...
<form name="historyForm" method="get" action="${ctxt}/search/searchPubmed.do" >	
	...
	<input type="hidden" id="pubSearchQuery" name="pubSearchQuery" value="${vo.pubSearchQuery}" />
</form>
...
<form name="reqForm" method="get" action="">
	...
	<div class="sub_center_text">
		...
		<span class="center_text_01">PubMed<span class="category_select">▼</span></span>
