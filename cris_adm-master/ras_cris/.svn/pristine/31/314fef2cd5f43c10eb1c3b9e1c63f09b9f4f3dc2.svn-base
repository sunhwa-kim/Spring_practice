...
<form name="reqForm" method="post">
	...
	<div class="sub_center_text">
		...
		<span class="center_text_01">보고서 상세</span>
	</div> 
		...
		<div class="content_box">
			...
			<div class="selectbox_wrap1" style="height:30px; position:relative; margin:0; padding-top:0; float: right; width: 100px;">									
				...
				<div class="list_title">
					<p>
					<c:choose>  
						<c:when test = "${not empty mlibVo.alternativetitle}"  >
							${mlibVo.title} = ${mlibVo.alternativetitle}
						</c:when>
						<c:otherwise>
							${mlibVo.title}
						</c:otherwise>
					...
					<div id="JOU_bigbox">
						...
						<div id="bibElement"> 
						    ...
						    <c:if test="${not empty mlibVo.grantId || not empty mlibVo.reportNo}">
							 ...
							 <c:if test="${not empty mlibVo.authorno || not empty mlibVo.instituteAbbr}">
								 <dl>
									<dt>도서기호</dt>
									<dd>
										${mlibVo.instituteAbbr}&nbsp;${mlibVo.authorno}
									</dd>
								 </dl>
							</c:if>
...
<%-- 						 	<input type="hidden" name="a" id="a" value="${a}"> --%>
					...
					<div class="tab-group mlibView" style="margin:40px auto 10px auto;">
								<ul class="tabs">
							        ...
							        <li rel="tab2" style="width:20%; box-sizing:border-box;"><!-- 보고서(발행년도) -->원문정보</li>
								</ul>
