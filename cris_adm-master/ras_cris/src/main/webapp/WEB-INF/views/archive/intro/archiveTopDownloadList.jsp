...
function fn_search(page){
	...
	****************************************************************/
    ...
    $.ajax({
        url: '${ctxt}/archive/intro/archiveTopDownloadListRead.do',
        data: params,
        type: 'POST',
        dataType: 'text',
        cache: false,
			...
			if(xmlList.length > 0){
				...
				xmlList.each(function(cnt){
					...
					****************************************************************/
					...
					if(archive_gbn == '1'){
						dataGbn = "report";
	        	 		html += '<td class = "a0 block" title = "">보고서</td>'; 										        	 		
	        	 	}else if(archive_gbn == '2'){
	        	 		dataGbn = "rnd";
	        	 		html += '<td class = "a0 block" title = "">논문</td>'; 	
	        	 	}
					/* if(archive_gbn == '1'){
						dataGbn = "report";
					...
					if(archive_sj.length>75){
						html += '<a href="javascript:fn_page(\''+archive_id+'\',\''+dataGbn+'\',\'download\')" >'+archive_sj.substr(0,75)+"..." +'</a></td>';
					}else{
						html += '<a href="javascript:fn_page(\''+archive_id+'\',\''+dataGbn+'\',\'download\')" >'+archive_sj +'</a></td>';
							
					}
					html += ' <td style ="text-align:center;">'+ downcnt + '</td>';
					html += '</tr>';
				...
				$("#topInfo").html("<span class = 'gray'>["+ cuurPage +" / "+pageTotalCnt+" Pages] </span> <span class = 'txt-red' >"+dataTotalCnt+"</span> 건이 검색되었습니다.</strong>");
			    $("#bodyInfo").html(html);
...
<style>
	...
	<div class="sub_center_text">
		<div class="sub_center_nav"> 
			...
			<span>아카이브</span>
			<span> > </span>
			<span class="active">인기자료</span>        
		</div>
	    <span class="center_text_01" >인기자료</span>
	</div> 
			...
			<div class="content_list">					
		        <div id="search-group">	        	
					<div class="search-input">
						<!-- <label for="searchCombo">검색콤보박스</label> -->
				...
				<div id="divRefreshArea">
					...
					<table class = "basic_table">
						<caption>
						 	 Recently Add Items - 번호 , 제목, 다운로드수 
						</caption>
						<colgroup>
<%-- 							<col style="width:6%">
							<col style="width:6%">
				...
				<div class="btn-group back">
					<a href="javascript:fn_returnListUrl()" class="basic-btn back">아카이브 시작페이지</a>
				</div> 	
			</div>		
		</div>
	</div>
